...
const initialTasks: Task[] = [
    { id: 'TASK-014', name: '华南线索转化率根因分析', priority: 'P0', status: '待处理', assignee: '线索诊断Agent', creator: '赵志强', deadline: '2小时内', description: '分析华南区域线索到店率连续3天低于全国均值15%的根因，2h内完成', projectId: 'PJ-LEAD-Q3-02' },
    { id: 'TASK-013', name: 'N7试驾链路优化方案输出', priority: 'P1', status: '待处理', assignee: '营销策略Agent', creator: '李明远', deadline: '今日 16:00 前', description: '基于用户画像和竞品对标，输出N7试驾预约链路优化方案，含3个改进方向' },
    { id: 'TASK-012', name: '客服对话质量抽查（50条）', priority: 'P2', status: '待处理', assignee: '王晓芳', creator: '赵志强', deadline: '今日', description: '随机抽样昨日50条客服对话，人工审核 Agent 应答质量并标记 badcase' },
    { id: 'TASK-010', name: 'Q3线索转化提升方案审批', priority: 'P0', status: '进行中', assignee: '李明远', creator: '营销策略Agent', deadline: '今日', description: '审核营销策略 Agent 生成的 Q3 方案，确认三大策略方向，批准执行', projectId: 'PJ-LEAD-Q3-02' },
    { id: 'TASK-009', name: '内容 Agent v3.0.3 用例修复', priority: 'P1', status: '进行中', assignee: '陈思敏', creator: 'Agent测试沙箱', deadline: '明日', description: '修复预发布测试中发现的8个失败用例：6个 Prompt 注入与2个平台适配问题', projectId: 'PJ-LEAD-OPS-01' },
    { id: 'TASK-007', name: 'Token 销售定价方案', priority: 'P1', status: '待审核', assignee: '张伟国', creator: '营销策略Agent', deadline: '今日', description: '三层 Token 套餐定价方案：基础版、专业版、企业版，需完成审批' },
    { id: 'TASK-006', name: 'N7城市NOA文档审核', priority: 'P2', status: '待审核', assignee: '王晓芳', creator: '知识库', deadline: '今日', description: '审核陈思敏提交的3篇 N7 城市 NOA 功能文档，确认内容准确性' },
    { id: 'TASK-005', name: '品牌方A晨间数据同步', priority: 'P2', status: '已完成', assignee: '数据同步Agent', creator: '系统自动', deadline: '2m10s', description: '完成品牌方 A/B/C 数据同步，耗时2分10秒', projectId: 'PJ-LEAD-OPS-01' },
];
...
export function M05TaskCenterPage({ role, handoff, onConsumeHandoff }: { role: RoleId; handoff?: WorkspaceHandoff; onConsumeHandoff?: () => void }) {
    ...
    const [draft, setDraft] = useState({ name: '', priority: 'P2' as TaskPriority, deadline: '', description: '', assigneeType: '人类员工', assignee: people[0], projectId: '' });
    ...
    return <section className="m05" data-module="task-center">
        ...
        </div> : <div className="m05-list"><table><thead><tr><th>编号</th><th>任务名称</th><th>关联项目</th><th>优先级</th><th>状态</th><th>指派人</th><th>创建者</th><th>截止时间</th><th>操作</th></tr></thead><tbody>
            {visibleTasks.map((task) => <tr key={task.id} onClick={() => { setDetail(task); setReassigning(false); }}><td>{task.id}</td><td><b>{task.name}</b></td><td>{projectName(task.projectId)}</td><td><span className={`m05-tag ${task.priority.toLowerCase()}`}>{task.priority}</span></td><td>{task.status}</td><td>{isAgent(task.assignee) ? '🤖' : '👤'} {task.assignee}</td><td>{task.creator}</td><td>{task.deadline}</td><td><button>详情</button></td></tr>)}
        </tbody></table></div>}
        </div>
        ...
        {createOpen && <div className="m05-mask" onMouseDown={closeCreate}><form onMouseDown={(event) => event.stopPropagation()} onSubmit={(event) => { event.preventDefault(); createTask(); }}>
            ...
            <label className="wide">关联项目（可选）<select value={draft.projectId} onChange={(event) => setDraft({ ...draft, projectId: event.target.value })}><option value="">不关联项目</option>{availableProjects.map((project) => <option key={project.id} value={project.id}>{project.name} · {project.id}</option>)}</select><small>仅显示当前角色有权访问且未关闭的 M10 项目。</small></label>
        ...
        {detail && <aside className="m05-drawer" aria-label="任务详情"><button className="m05-drawer-close" onClick={() => setDetail(null)}>关闭</button><h2>📋 {detail.id}</h2>
            <section><h3>任务详情</h3><div className="m05-kpis"><span><b className={detail.priority.toLowerCase()}>{detail.priority}</b><small>优先级</small></span><span><b>{detail.status}</b><small>状态</small></span><span><b>{detail.id}</b><small>编号</small></span></div>
                <dl><dt>任务名称</dt><dd>{detail.name}</dd><dt>指派人</dt><dd>{detail.assignee}</dd><dt>创建者</dt><dd>{detail.creator}</dd><dt>截止时间</dt><dd>{detail.deadline}</dd><dt>关联项目</dt><dd>{projectName(detail.projectId)}{detail.projectId&&<small> · {detail.projectId}</small>}</dd><dt>描述</dt><dd>{detail.description}</dd></dl>
