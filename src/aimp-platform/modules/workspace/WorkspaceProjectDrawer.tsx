    ...
    return <div className="m04-overlay" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
        <section className="m04-project-drawer" role="dialog" aria-modal="true" aria-label="项目详情">
            ...
            <div className="m04-project-content">
                ...
                {tab === 'milestones' && <><div className="m04-tab-heading"><Milestone size={18} /><div><strong>项目里程碑</strong><small>展示计划节点、状态和前后依赖</small></div></div><RecordList records={workspaceMilestones} /></>}
                ...
                {tab === 'members' && <><div className="m04-tab-heading"><Users size={18} /><div><strong>项目成员</strong><small>组织、岗位、流程职责与协作关系</small></div></div><RecordList records={workspaceMembers} /></>}
                {tab === 'agents' && <><div className="m04-tab-heading"><Bot size={18} /><div><strong>绑定 Agent</strong><small>当前版本、用途、状态和项目授权</small></div></div><div className="m04-record-list">{workspaceAgents.slice(0, 3).map((agent) => <button type="button" key={agent.id} onClick={() => onDialog({ id: 'agent-detail', payload: agent.id })}><span><strong>{agent.name} · {agent.version}</strong><small>{agent.status} · {agent.owner} · {agent.permission}</small></span><em>查看</em><ChevronRight size={15} /></button>)}</div></>}
                {tab === 'resources' && <><div className="m04-lineage"><span>{project.id}</span><ChevronRight size={15} /><span>参与 Agent</span><ChevronRight size={15} /><span>知识 / 数据 / 内容</span><ChevronRight size={15} /><span>会话与任务证据</span></div><div className="m04-binding-groups">{(['knowledge', 'data', 'content'] as const).map((kind) => {
                    ...
                    return <section key={kind}><header><Icon size={18} /><div><strong>{label}</strong><small>{kind === 'knowledge' ? '经治理的事实、规则与RAG资产' : kind === 'data' ? '结构化指标、事件和脱敏样本' : '经内容中心审核的文案、话术和素材包'}</small></div></header>{workspaceBindings.filter((item) => item.kind === kind).map((binding) => <button type="button" key={binding.id} onClick={() => onDialog({ id: 'source-detail', payload: binding.id })}><dl><div><dt>资产 ID</dt><dd>{binding.id}</dd></div><div><dt>版本</dt><dd>{binding.version}</dd></div><div><dt>状态</dt><dd>{binding.status}</dd></div><div><dt>用途</dt><dd>{binding.purpose}</dd></div><div><dt>消费 Agent</dt><dd>{binding.consumerAgentIds.join('、')}</dd></div><div><dt>授权范围</dt><dd>{binding.access}</dd></div><div><dt>使用证据</dt><dd>{binding.evidence}</dd></div></dl><ChevronRight size={16} /></button>)}</section>;
                })}</div></>}
                {tab === 'delivery' && <div className="m04-delivery-summary"><ShieldCheck size={25} /><h3>项目关联交付合同 {project.deliveryId}</h3><p>当前角色仅查看交付摘要；合同金额、计费规则和客户敏感字段按权限隐藏。</p><dl><div><dt>交付产品</dt><dd>线索智能运营套件 v2.1</dd></div><div><dt>服务模式</dt><dd>RaaS</dd></div><div><dt>当前SLA</dt><dd>98.6% · 合同目标≥97%</dd></div><div><dt>项目关系</dt><dd>本项目是合同下4个子项目之一</dd></div><div><dt>受限字段</dt><dd>合同金额、计费单价、客户联系人已隐藏</dd></div></dl></div>}
